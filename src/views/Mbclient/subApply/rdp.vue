<template>
    <div>
        <ul class="ula">
            <li>
                <p class="p_left">RDP应用</p>
                <p class="p_right" @click="lookRdp" v-if="isShowRdp==false?true:false" style="font-size: 20px;margin-right:10px"> > </p>
                <p class="p_right" @click="lookRdp" v-if="isShowRdp==true?true:false" style="font-size: 20px;margin-right:10px"> ∨ </p>
            </li>
            <li v-for="(item,index) in rdpList" v-if="isShowRdp" :key="index">
...
<script>
    ...
    export default {
        ...
        methods: {
            lookRdp(){
                if(this.isShowRdp==false){
                    this.isShowRdp=true
                }else {
                    this.isShowRdp=false
                }
            },
            collection(item){
                this.$emit('rdpCollection',item)
            }
